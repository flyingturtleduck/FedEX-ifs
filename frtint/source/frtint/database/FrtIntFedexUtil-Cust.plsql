...
BEGIN
   ...
   IF token_ IS NULL THEN
      ...
      Frt_Transaction_API.Set_Error_Message(transaction_id_, error_message_);
      RETURN;
   END IF;
   ...
   IF err_msg_ IS NOT NULL THEN
      ...
      IF log_trace_ = 'TRUE' THEN
         Frt_Int_Ship_Util_API.Log_Shipment_Trace(
            ...
            error_message_       => err_msg_);
      END IF;
      Frt_Transaction_API.Set_Error_Message(transaction_id_, SUBSTR(err_msg_, 1, 2000));
      RETURN;
   END IF;

   shipment_trans_url_ := confirm_rec_.master_tracking_number;
   Frt_Transaction_API.Modify_Provider_Url(transaction_id_, shipment_trans_url_);
...
EXCEPTION
   WHEN OTHERS THEN
      ...
      Frt_Transaction_API.Set_Error_Message(transaction_id_, SUBSTR(error_message_, 1, 2000));
END Send_Shipment;
...
BEGIN
   ...
   shipment_rates_.rating_list := Frt_Int_FedEx_Message_Util_API.Json_To_Sr_Rate_Arr(fe_rates_);
EXCEPTION
   WHEN OTHERS THEN
      error_message_ := SQLERRM;
...
BEGIN
   ...
   IF NOT success_ THEN
      Frt_Transaction_API.Set_Error_Message(transaction_id_, 'FedEx Process_Shipment: re-confirm failed.');
      RETURN;
   END IF;
...
EXCEPTION
   WHEN OTHERS THEN
      Frt_Transaction_API.Set_Error_Message(transaction_id_, SQLERRM);

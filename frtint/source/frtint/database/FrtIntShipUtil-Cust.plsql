...
BEGIN
   IF provider_ LIKE 'FEDEX%' THEN
      ...
      Frt_Transaction_Util_API.Update_Transaction(
         ...
         tracking_number_ => NVL(fe_confirm_.tracking_number, fe_confirm_.master_tracking_number),
         freight_charge_  => fe_confirm_.net_charge_amount,
         currency_code_   => fe_confirm_.currency);
      --(+) 260819  I006 (START)
...
BEGIN
   CASE status_
      WHEN 'Completed' THEN
         Frt_Transaction_API.Set_Completed(
            source_ref_type_  => source_ref_type_,
            source_ref_       => source_ref_,
            freight_msg_type_ => freight_msg_type_);
      WHEN 'Confirmed' THEN
         Frt_Transaction_API.Set_Confirmed(
            source_ref_type_  => source_ref_type_,
            source_ref_       => source_ref_,
            freight_msg_type_ => freight_msg_type_);
      WHEN 'Pending' THEN
         Frt_Transaction_API.Set_Pending(
            source_ref_type_  => source_ref_type_,
            source_ref_       => source_ref_,
            freight_msg_type_ => freight_msg_type_);
      WHEN 'Void' THEN
         Frt_Transaction_API.Set_Void(
            source_ref_type_  => source_ref_type_,
            source_ref_       => source_ref_,
            freight_msg_type_ => freight_msg_type_);
      ELSE NULL;
...
BEGIN
   ...
   Frt_Int_Log_Trace_API.New_Update(
      ...
      status_              => status_,
      error_message_       => error_message_);
   IF status_ = 'Error' AND Frt_Transaction_API.Exists(transaction_id_) THEN

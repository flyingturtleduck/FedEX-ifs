-----------------------------------------------------------------------------
--
--  Logical unit: FrtIntFedexMessageUtil
--  Component:    FRTINT
--
...
--  260826    Fixed rate parsing: ACCOUNT/LIST filter, arrival_date, carrier_svc_code, service_recommendation
--  260826    Renamed LU FedexMessageUtil -> FrtIntFedexMessageUtil to follow FRTINT naming convention
-----------------------------------------------------------------------------
...
TYPE Fe_Rate_Rec IS RECORD (
   ...
   arrival_date                 DATE,
   transit_time                 NUMBER,
   carrier_svc_code             VARCHAR2(50),
   list_net_charge              NUMBER);
...
BEGIN
   ...
   rec_.address.country_code           := Company_Address_API.Get_Country_Db(company_, addr_key_);
   rec_.address.residential            := 'false';
   rec_.contact.company_name           := Company_API.Get_Name(company_);
   rec_.contact.phone_number           := Comm_Method_API.Get_Value('COMPANY', company_, Comm_Method_Code_API.Decode('PHONE'), 1, addr_key_, SYSDATE);
   rec_.contact.email_address          := Comm_Method_API.Get_Value('COMPANY', company_, Comm_Method_Code_API.Decode('E_MAIL'), 1, addr_key_, SYSDATE);
   RETURN rec_;
END Get_Shipper_Rec;
...
BEGIN
   ...
   CLOSE get_shipment;
   rec_.address.address_line_1         := ship_.receiver_address1;
   rec_.address.address_line_2         := ship_.receiver_address2;
   rec_.address.city                   := ship_.receiver_city;
   rec_.address.state_or_province_code := ship_.receiver_state;
   rec_.address.postal_code            := ship_.receiver_zip_code;
   rec_.address.country_code           := ship_.receiver_country;
   rec_.contact.company_name           := ship_.receiver_address_name;
   rec_.contact.person_name            := ship_.receiver_reference;
   rec_.contact.phone_number           := Comm_Method_API.Get_Default_Value(
                                            'CUSTOMER', ship_.receiver_id,
                                            Comm_Method_Code_API.Decode('PHONE'),
                                            ship_.receiver_addr_id, SYSDATE);
   rec_.contact.email_address          := Comm_Method_API.Get_Default_Value(
                                            'CUSTOMER', ship_.receiver_id,
...
BEGIN
   ...
   rec_.address.postal_code            := addr_.zip_code;
   rec_.address.country_code           := addr_.country_code;
   rec_.contact.company_name           := Customer_Info_API.Get_Name(customer_no_);
   rec_.contact.phone_number           := Comm_Method_API.Get_Default_Value(
                                            'CUSTOMER', customer_no_,
                                            Comm_Method_Code_API.Decode('PHONE'),
                                            addr_no_, SYSDATE);
   rec_.contact.email_address          := Comm_Method_API.Get_Default_Value(
                                            'CUSTOMER', customer_no_,
...
BEGIN
   ...
   FOR hu_ IN get_hu LOOP
      ...
      items_(idx_).weight.value          := NVL(hu_.manual_gross_weight,
                                                NVL(Handling_Unit_API.Get_Operative_Gross_Weight(hu_.handling_unit_id), 0));
...
IS
   ...
   CURSOR get_lines IS
      SELECT col.catalog_no,
             ...
             Inventory_Part_API.Get_Country_Of_Origin(col.contract, col.part_no) AS country_of_manufacture,
       ...
       WHERE col.order_no    = Shipment_API.Get_One_Distinct_Source_Ref1(shipment_id_, Logistics_Source_Ref_Type_API.DB_CUSTOMER_ORDER)
...
FUNCTION Get_Order_Create_Ship_Req_Rec (
   freight_provider_  IN VARCHAR2,
...
BEGIN
   ...
   req_.requested_shipment.include_customs := FALSE;
   RETURN req_;
END Get_Order_Create_Ship_Req_Rec;
...
IS
   ...
   svc_desc_obj_    Json_Object_T;
   op_detail_obj_   Json_Object_T;
   date_detail_obj_ Json_Object_T;
   idx_             PLS_INTEGER := 1;
   base_charge_     NUMBER := 0;
   net_charge_      NUMBER := 0;
   list_charge_     NUMBER := 0;
   fuel_pct_        NUMBER := 0;
   currency_        VARCHAR2(3);
   carrier_svc_      VARCHAR2(50);
   transit_days_     VARCHAR2(30);
   arrival_date_     DATE;
   transit_time_     NUMBER;
   commit_dt_str_    VARCHAR2(50);
   rate_type_        VARCHAR2(30);
BEGIN
   ...
   FOR i_ IN 0..details_arr_.get_size - 1 LOOP
      ...
      FOR j_ IN 0..rated_arr_.get_size - 1 LOOP
         rated_obj_  := TREAT(rated_arr_.get(j_) AS Json_Object_T);
         IF rated_obj_ IS NULL THEN CONTINUE; END IF;
         rate_type_ := rated_obj_.get_string('rateType');
         IF rate_type_ = 'ACCOUNT' THEN
            base_charge_   := NVL(rated_obj_.get_number('totalBaseCharge'), 0);
            net_charge_    := NVL(rated_obj_.get_number('totalNetCharge'), 0);
            currency_      := rated_obj_.get_string('currency');
            ...
            END IF;
         ELSIF rate_type_ = 'LIST' THEN
            list_charge_ := NVL(rated_obj_.get_number('totalNetCharge'), 0);
         END IF;
      END LOOP;

      IF net_charge_ = 0 AND list_charge_ = 0 THEN CONTINUE; END IF;

      -- carrier_svc_code: serviceDescription.code → operationalDetail.serviceCode
      carrier_svc_  := NULL;
      svc_desc_obj_ := TREAT(detail_obj_.get('serviceDescription') AS Json_Object_T);
      IF svc_desc_obj_ IS NOT NULL THEN
         carrier_svc_ := svc_desc_obj_.get_string('code');
      END IF;
      IF carrier_svc_ IS NULL THEN
         op_detail_obj_ := TREAT(detail_obj_.get('operationalDetail') AS Json_Object_T);
         IF op_detail_obj_ IS NOT NULL THEN
            carrier_svc_ := op_detail_obj_.get_string('serviceCode');
         END IF;
      END IF;

      -- arrival_date: commit.dateDetail.dayCxsFormat → transit_days from transitDays/daysInTransit
      arrival_date_ := NULL;
      transit_days_ := NULL;
      commit_obj_   := TREAT(detail_obj_.get('commit') AS Json_Object_T);
      IF commit_obj_ IS NOT NULL THEN
         date_detail_obj_ := TREAT(commit_obj_.get('dateDetail') AS Json_Object_T);
         IF date_detail_obj_ IS NOT NULL THEN
            commit_dt_str_ := date_detail_obj_.get_string('dayCxsFormat');
            IF commit_dt_str_ IS NOT NULL THEN
               BEGIN
                  arrival_date_ := TO_DATE(SUBSTR(commit_dt_str_, 1, 19), 'YYYY-MM-DD"T"HH24:MI:SS');
               EXCEPTION
                  WHEN OTHERS THEN arrival_date_ := NULL;
               END;
            END IF;
         END IF;
         IF arrival_date_ IS NULL THEN
      ...
      rates_(idx_).arrival_date           := arrival_date_;
      rates_(idx_).transit_time           := transit_time_;
      rates_(idx_).carrier_svc_code       := carrier_svc_;
      rates_(idx_).list_net_charge        := list_charge_;
      idx_ := idx_ + 1;
   END LOOP;
   RETURN rates_;
...
FUNCTION Json_To_Sr_Rate_Arr (
   fe_arr_ IN Fe_Rate_Arr) RETURN Sr_Shipment_Rates_Rec
IS
   sr_rec_      Sr_Shipment_Rates_Rec;
   i_           PLS_INTEGER;
   min_charge_  NUMBER       := NULL;
   min_arrival_ DATE         := NULL;
   lc_idx_      PLS_INTEGER  := NULL;
   fa_idx_      PLS_INTEGER  := NULL;
BEGIN
   IF fe_arr_.COUNT = 0 THEN RETURN sr_rec_; END IF;

   i_ := fe_arr_.FIRST;
   WHILE i_ IS NOT NULL LOOP
      sr_rec_.rating_list(i_).carrier_service_code             := fe_arr_(i_).service_type;
      sr_rec_.rating_list(i_).carrier_service_code_description := fe_arr_(i_).service_name;
      sr_rec_.rating_list(i_).consignor_freight                := fe_arr_(i_).total_net_charge;
      sr_rec_.rating_list(i_).list_freight                     := fe_arr_(i_).list_net_charge;
      sr_rec_.rating_list(i_).fuel_surcharge                   := ROUND(fe_arr_(i_).total_base_charge * fe_arr_(i_).fuel_surcharge_percent / 100, 2);
      sr_rec_.rating_list(i_).currency_code                    := fe_arr_(i_).currency;
      sr_rec_.rating_list(i_).transit_time                     := fe_arr_(i_).transit_time;
      sr_rec_.rating_list(i_).arrival_date_text                := TO_CHAR(fe_arr_(i_).arrival_date, 'YYYY-MM-DD');

      IF fe_arr_(i_).total_net_charge > 0 AND (min_charge_ IS NULL OR fe_arr_(i_).total_net_charge < min_charge_) THEN
         min_charge_ := fe_arr_(i_).total_net_charge;
         lc_idx_     := i_;
      END IF;

      IF fe_arr_(i_).arrival_date IS NOT NULL AND (min_arrival_ IS NULL OR fe_arr_(i_).arrival_date < min_arrival_) THEN
         min_arrival_ := fe_arr_(i_).arrival_date;
         fa_idx_      := i_;
      END IF;
   ...
   IF lc_idx_ IS NOT NULL THEN
      sr_rec_.service_recommendation.lowest_cost_carrier_service_code             := fe_arr_(lc_idx_).service_type;
      sr_rec_.service_recommendation.lowest_cost_carrier_service_code_description := fe_arr_(lc_idx_).service_name;
      sr_rec_.service_recommendation.lowest_cost_consignor_freight                := fe_arr_(lc_idx_).total_net_charge;
      sr_rec_.service_recommendation.lowest_cost_list_freight                     := fe_arr_(lc_idx_).list_net_charge;
      sr_rec_.service_recommendation.lowest_cost_fuel_surcharge                   := ROUND(fe_arr_(lc_idx_).total_base_charge * fe_arr_(lc_idx_).fuel_surcharge_percent / 100, 2);
      sr_rec_.service_recommendation.lowest_cost_currency_code                    := fe_arr_(lc_idx_).currency;
      sr_rec_.service_recommendation.lowest_cost_arrival_date_text                := TO_CHAR(fe_arr_(lc_idx_).arrival_date, 'YYYY-MM-DD');
      sr_rec_.service_recommendation.lowest_cost_transit_time                     := fe_arr_(lc_idx_).transit_time;
   END IF;

   IF fa_idx_ IS NOT NULL THEN
      sr_rec_.service_recommendation.fastest_carrier_service_code                 := fe_arr_(fa_idx_).service_type;
      sr_rec_.service_recommendation.fastest_carrier_service_code_description     := fe_arr_(fa_idx_).service_name;
      sr_rec_.service_recommendation.fastest_consignor_freight                    := fe_arr_(fa_idx_).total_net_charge;
      sr_rec_.service_recommendation.fastest_list_freight                         := fe_arr_(fa_idx_).list_net_charge;
      sr_rec_.service_recommendation.fastest_fuel_surcharge                       := ROUND(fe_arr_(fa_idx_).total_base_charge * fe_arr_(fa_idx_).fuel_surcharge_percent / 100, 2);
      sr_rec_.service_recommendation.fastest_currency_code                        := fe_arr_(fa_idx_).currency;
      sr_rec_.service_recommendation.fastest_arrival_date_text                    := TO_CHAR(fe_arr_(fa_idx_).arrival_date, 'YYYY-MM-DD');
      sr_rec_.service_recommendation.fastest_transit_time                         := fe_arr_(fa_idx_).transit_time;
   END IF;
